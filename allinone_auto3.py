...
def auth():
    ...
    file_name = "allinone_auto_email.xlsx"
    get_file_from_gdrive(cwd, file_name)

    file_path = f'data/{file_name}'
    df_email = pd.read_excel(file_path)

    if user_email not in list(df_email['email']):
        st.write('承認されたメールアドレスではありません。')
        link = '[ログイン画面](https://cocosan1-hidastreamlit3-allinone-auto-test-glx9s6.streamlit.app/)'
        ...
        data_list = ['--データを選択--', '全国', '北日本', '東日本', '中部', '関西', '中四国九州']
        ...
        data_dict = {
            '北日本': ['kita79j', 'kita78j'],
            '東日本': ['higashi79j', 'higashi78j'],
            '中部': ['chuubu79j', 'chuubu78j'],
            '関西': ['kansai79j', 'kansai78j'],
            '中四国九州': ['chuushikoku79j', 'chuushikoku78j']
        }

        if selected_data == '全国':
            #全国データの作成
            area_list = data_list.copy()
            del area_list[0:2] #'--データを選択--', '全国'を削除

            for area in area_list:
                fname_list = data_dict[area]
            ...
            fname_list = data_dict[selected_data]
            for fname in fname_list:
        ...
        if selected_data == '全国':
            ...
            for area in area_list:
                ...
                # ***今期受注***
                #受注データのpath取得
                fname_jnow = f'{fname_list[0]}.xlsx'
                path_jnow = os.path.join(cwd, 'data', fname_jnow)

                df_now2 = make_data_now(path_jnow)

                # ***前期受注***
                fname_jlast = f'{fname_list[1]}.xlsx'
                path_jlast = os.path.join(cwd, 'data', fname_jlast)

                df_last2 = make_data_last(path_jlast)

                df_now = pd.concat([df_now, df_now2], join='outer')
                df_last = pd.concat([df_last, df_last2], join='outer')
            ...
            df_last = make_data_last(path_jlast)
        ...
        end_day = df_now['受注日'].max()
        #時刻の削除
        end_day2 = str(end_day).split(' ')[0]
        end_day2 = end_day2.replace('-', '/')

        st.sidebar.write('受注ベース 期間：')
        st.sidebar.write(f'{start_day2}-{end_day2}')
        ...
        # #**********************************************************棒グラフ　今期前期
        # def make_bar_nowlast(lists_now, lists_last, x_list):
        #     #可視化
        #     #グラフを描くときの土台となるオブジェクト
        #     fig = go.Figure()
        #     #今期のグラフの追加
        ...
        # #**********************************************************棒グラフ　今期前期 小数
        # def make_bar_nowlast_float(lists_now, lists_last, x_list):
        #     #可視化
        #     #グラフを描くときの土台となるオブジェクト
        #     fig = go.Figure()
        #     #今期のグラフの追加
        ...
        # #**********************************************************折れ線
        # def make_line(df_list, name_list):
        #     #グラフを描くときの土台となるオブジェクト
        #     fig = go.Figure()
        #     #今期のグラフの追加
